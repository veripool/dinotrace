...
(defconst dinotrace-mode-version "$$Revision$$"
...
(defun dinotrace-make-propertied (sig)
  ...
  (let* ((values (dinotrace-signal-values sig))
    ...
    (when (dinotrace-signal-color sig)
      (set-text-properties 0 (length out)
			   (list `face (dinotrace-sigcolor-face
    ...
    (while values
      ...
      (set-text-properties oldlen (length out)
			   (list `face (dinotrace-cursor-face cursor-num))
			   out)
...
(defun dinotrace-highlight-signal (point &optional color)
  ...
  (let ((signal (dinotrace-find-signal-default point)))
    (when signal
      (dinotrace-send-command
	(format "signal_highlight *%s%s[* %d\n" dinotrace-hierarchy-separator signal color)
	(format "signal_highlight *%s%s %d\n" dinotrace-hierarchy-separator signal color)
	(format "signal_highlight %s[* %d\n" signal color)
	(format "signal_highlight %s %d\n" signal color)
	"refresh\nannotate\n")
